...
class JustSquares : public olc::PixelGameEngine {
	...
	bool OnUserCreate() override {
		...
		invslotimage = new olc::Sprite("game/internal/textures/inventory-slot.png");
		invslot = new olc::Decal(invslotimage);
		selslotimage = new olc::Sprite("game/internal/textures/selected-slot.png");
		...
		textureimage = new olc::Sprite("./game/packs/default/textures/blocks.png");
		watertimage = new olc::Sprite("./game/internal/textures/water.png");
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (playmode == "moving" && sysmode != "server") {
			...
			//	entities[currplayer]->applyforce({0, 0.1});
			//}
		...
		bool sendpack = false;
		if (w.blocks[(int)(entities[currplayer]->pos.x)][(int)(entities[currplayer]->pos.y)].waterlevel > 3) {
			sendpack = entities[currplayer]->update(0.15, 0.2);
		} else {
			sendpack = entities[currplayer]->update(0.3, 0.45);
		}
		...
		for (int i = 0; i < 10; i++) {
			if (currinvbar == 0) {
				if (i == currinvslot) {
					DrawDecal({32 + (24*i), 8}, selslot, {1.0, 1.0}, olc::Pixel(255, 63, 63));
				} else {
					DrawDecal({32 + (24*i), 8}, invslot, {1.0, 1.0}, olc::Pixel(255, 63, 63));
				}
			}
			if (currinvbar == 1) {
				if (i == currinvslot) {
					DrawDecal({32 + (24*i), 8}, selslot, {1.0, 1.0}, olc::Pixel(255, 255, 63));
				} else {
					DrawDecal({32 + (24*i), 8}, invslot, {1.0, 1.0}, olc::Pixel(255, 255, 63));
				}
			}
			if (currinvbar == 2) {
				if (i == currinvslot) {
					DrawDecal({32 + (24*i), 8}, selslot, {1.0, 1.0}, olc::Pixel(63, 255, 63));
				} else {
					DrawDecal({32 + (24*i), 8}, invslot, {1.0, 1.0}, olc::Pixel(63, 255, 63));
				}
			}
			if (currinvbar == 3) {
				if (i == currinvslot) {
					DrawDecal({32 + (24*i), 8}, selslot, {1.0, 1.0}, olc::Pixel(63, 127, 255));
				} else {
					DrawDecal({32 + (24*i), 8}, invslot, {1.0, 1.0}, olc::Pixel(63, 127, 255));
				}
			}
			olc::vf2d relpos;
